...
def test_normal_activity(
    ...
    rai.approve(vault, 2 ** 256 - 1, {"from": whale})
    rai.approve(vault, 2 ** 256 - 1, {"from": strategist})
...
def test_debt_increase(
    rai, Strategy, chain, whale, gov, strategist, rando, vault, strategy, fn_isolation
):
...
def test_good_migration(
    rai, Strategy, chain, whale, gov, strategist, rando, vault, strategy, fn_isolation, crRai
):
    currency = rai
