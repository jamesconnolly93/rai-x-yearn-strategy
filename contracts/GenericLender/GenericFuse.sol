...
/********************
 *   A lender plugin for LenderYieldOptimiser for any erc20 asset on Fuse (not eth)
 *   Made by SamPriestley.com
 *
 ********************* */

contract GenericFuse is GenericLenderBase {
    ...
    function _initialize(address _cToken) internal {
        require(address(cToken) == address(0), "GenericFuse already initialized");
        cToken = CErc20I(_cToken);
        require(cToken.underlying() == address(want), "WRONG CTOKEN");
        want.safeApprove(_cToken, uint256(-1));
    }

    function cloneFuseLender(
        address _strategy,
        string calldata _name,
        address _cToken
    ) external returns (address newLender) {
        newLender = _clone(_strategy, _name);
        GenericFuse(newLender).initialize(_cToken);
    }
